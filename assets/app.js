(function () {
  ...
  function openDetail(o) {
    ...
    $("#d-body").innerHTML = `<div class="d-grid">
      <div><div class="dimg">${imageSources(o, true).length ? imgTag(o, true) : `<div class="thumb">${placeholder(o)}</div>`}</div>${o.imageCredit && o.image ? `<p class="meta small">${esc(o.imageCredit)}</p>` : ""}${o.source === "British Museum" && o.image ? `<p class="meta small">Image © The Trustees of the British Museum, CC BY-NC-SA 4.0, shown from the Museum&#39;s server.</p>` : ""}${o.thumb ? `<p class="meta small">Image copy hosted with this catalogue under its open licence (${esc(o.dataLicence)}).</p>` : ""}</div>
      <div>
        ...
        ${o.harUrl ? `<p class="meta small"><a href="${esc(o.harUrl)}" target="_blank" rel="noopener">Himalayan Art Resources record ${esc(o.har)}</a></p>` : ""}
      </div></div>`;
    $("#detail").showModal();
  }
  ...
  $("#about").innerHTML = `
    ...
    <p>The British Museum, Musée Guimet, LACMA, the Museum of Fine Arts Boston, are listed under Institutions but not harvested.</p>
    <h2>Images</h2>
    <p>Openly licensed images from the Met, Cleveland, Wellcome, the Smithsonian and LACMA (public-domain works) are served as small copies stored with this catalogue (in one folder per institution), so they load even if the original server changes its rules. V&amp;A, Harvard, British Museum, Ashmolean (with permission) and Rubin (under its terms for non-commercial educational use) images are shown live from the institutions' own servers, credited on each record and may not always load. The Art Institute of Chicago's image server currently blocks display on other websites (it returns a Cloudflare challenge and a same-origin resource policy), so Chicago records show only a blurred colour preview supplied by its API; the full image is on the museum's own page. The Asian Art Museum's images sit behind a bot challenge and cannot be shown. Photographs of the French (Joconde) and Guimet records are shown live with the rights holders' permission.</p>
    <h2>Downloads</h2>
